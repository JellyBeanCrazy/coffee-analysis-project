...
We decided that this would be the most appropriate license for this project as it allows the client to freely use, modify, and re-run the analysis code themselves, which is a core requirement outlined in the brief. It is also compatible with all the open source libraries used in the project such as matplotlib, geopandas, and pandas, avoiding any licensing conflicts. Additionally, its simplicity makes it easy for all stakeholders, including the client who may not have a technical background, to understand what they are permitted to do with the code.
